...
import { CargoType, Country, Ship, ShipOperation, ShippingLine } from '@/types/ship-operations';
...
interface EditShipOperationProps {
...
export default function EditShipOperation({ operation, countries, shippingLines, cargoTypes, ships }: EditShipOperationProps) {
    const { data, setData, processing, errors } = useForm<ShipOperationFormData>({
        ship_id: operation.ship.id.toString(),
        cargo_type_id: operation.cargo_type.id.toString(),
        operation_date: operation.operation_date,
        unloading_tonnage: formatNumberDisplay(operation.unloading_tonnage),
        loading_tonnage: formatNumberDisplay(operation.loading_tonnage),
    ...
    // Initialize date from existing operation data
    useEffect(() => {
        if (data.operation_date) {
            try {
                // Try different date formats that might come from the backend
                let parsedDate: Date | null = null;
                // Try ISO format first (YYYY-MM-DD)
                if (data.operation_date.match(/^\d{4}-\d{2}-\d{2}$/)) {
                    parsedDate = parse(data.operation_date, 'yyyy-MM-dd', new Date());
                }
                // Try with time (YYYY-MM-DD HH:mm:ss or YYYY-MM-DDTHH:mm:ss)
                else if (data.operation_date.includes('T') || data.operation_date.includes(' ')) {
                    parsedDate = new Date(data.operation_date);
                }
                // Try other formats
                else {
                    parsedDate = new Date(data.operation_date);
                ...
                if (parsedDate && !isNaN(parsedDate.getTime())) {
                    setOperationDate(parsedDate);
                } else {
                    console.warn('Invalid date format:', data.operation_date);
                }
            } catch (error) {
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        router.put(route('user.ship-operations.update', operation.id), submitData, {
            onSuccess: () => {
    ...
    return (
        <AppLayout>
            <Head title="Edit Ship Operation" />
            <div className="p-6">
                <Card className="border-0 shadow-sm">
                    <CardHeader className="pb-4">
                        <CardTitle className="text-2xl font-semibold">Ubah Data Operasi Kapal</CardTitle>
                        <p className="mt-1 text-sm text-gray-600">Perbarui informasi operasi kapal {operation.ship.name}</p>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-6">
                                        ...
                                        <PopoverTrigger asChild>
                                            <Button
                                                ...
                                                className={cn(
                                                    ...
                                                    errors.operation_date && 'border-red-500 focus:border-red-500',
                                                )}
                                            >
                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                {operationDate && isValid(operationDate) ? (
                                                    format(operationDate, 'dd MMMM yyyy', { locale: id })
                                                ) : (
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    id="remarks"
                                    placeholder="Masukkan catatan tambahan tentang operasi kapal..."
                                    value={data.remarks}
                                    onChange={(e) => setData('remarks', e.target.value)}
                                    className={cn('min-h-[100px] resize-none', errors.remarks && 'border-red-500 focus:border-red-500')}
                                />
                            ...
                            <div className="flex justify-end gap-3 border-t pt-4">
                                ...
                                <Button type="submit" disabled={processing || isSubmitting} className="px-6">
